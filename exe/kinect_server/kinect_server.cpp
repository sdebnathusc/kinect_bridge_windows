...
typedef KinectFacesMessage _FacesMsg;
typedef std::shared_ptr<_FacesMsg> _FacesMsgPtr;

typedef KinectHDFacesMessage _HDFacesMsg;
typedef std::shared_ptr<_HDFacesMsg> _HDFacesMsgPtr;
...
//                std::cout << "pulling bodies" << std::endl;
				...
				//std::cout << "Body Success" << std::endl;
                output_fifo_.getHandle( atomics::HandleBase::NotifyType::DELAY_NOTIFY_ALL )->push_back( message_ptr );
            }
            catch( KinectException & e )
            {
...
class FacesReadTask : public Poco::Runnable
{
public:
	typedef atomics::Wrapper<std::deque<_FacesMsgPtr> > _OutputFifo;

	_OutputFifo & output_fifo_;
	KinectDevice & kinect_device_;
	bool running_;

	FacesReadTask(_OutputFifo & output_fifo, KinectDevice & kinect_device)
		:
		output_fifo_(output_fifo),
		kinect_device_(kinect_device),
		running_(true)
	...
	void run()
	{
		while (running_)
		{
			...
			try
			{
				//                std::cout << "pulling faces" << std::endl;
				_FacesMsgPtr message_ptr;
				kinect_device_.pullFaces(message_ptr);
				//std::cout << "Body Success" << std::endl;
				output_fifo_.getHandle(atomics::HandleBase::NotifyType::DELAY_NOTIFY_ALL)->push_back(message_ptr);
			}
			catch (KinectException & e)
...
class HDFacesReadTask : public Poco::Runnable
{
public:
	typedef atomics::Wrapper<std::deque<_HDFacesMsgPtr> > _OutputFifo;

	_OutputFifo & output_fifo_;
	KinectDevice & kinect_device_;
	bool running_;

	HDFacesReadTask(_OutputFifo & output_fifo, KinectDevice & kinect_device)
		:
		output_fifo_(output_fifo),
		kinect_device_(kinect_device),
		running_(false)
	...
	void run()
	{
		while (running_)
		{
			...
			try
			{
				//                std::cout << "pulling hd faces" << std::endl;
				_HDFacesMsgPtr message_ptr;
				kinect_device_.pullHDFaces(message_ptr);
				//std::cout << "Body Success" << std::endl;
				output_fifo_.getHandle(atomics::HandleBase::NotifyType::DELAY_NOTIFY_ALL)->push_back(message_ptr);
			}
			catch (KinectException & e)
...
class FacesCompressTask : public Poco::Runnable
{
public:
	typedef atomics::Wrapper<std::deque<_FacesMsgPtr> > _InputFifo;
	typedef atomics::Wrapper<std::deque<_CodedMsgPtr> > _OutputFifo;
	typedef MessageCoder<BinaryCodec<> > _MessageCoder;
	...
	FacesCompressTask(_InputFifo & input_fifo, _OutputFifo & output_fifo, _MessageCoder & message_coder)
		:
		input_fifo_(input_fifo),
		output_fifo_(output_fifo),
		message_coder_(message_coder),
	...
	void run()
	{
		while (running_)
		{
			...
			//            std::cout << "compressing faces" << std::endl;
		...
		//        std::cout << "!! FacesCompressTask done" << std::endl;
	}
};


class HDFacesCompressTask : public Poco::Runnable
{
public:
	typedef atomics::Wrapper<std::deque<_HDFacesMsgPtr> > _InputFifo;
	typedef atomics::Wrapper<std::deque<_CodedMsgPtr> > _OutputFifo;
	typedef MessageCoder<BinaryCodec<> > _MessageCoder;
	...
	HDFacesCompressTask(_InputFifo & input_fifo, _OutputFifo & output_fifo, _MessageCoder & message_coder)
		:
		input_fifo_(input_fifo),
		output_fifo_(output_fifo),
		message_coder_(message_coder),
	...
	void run()
	{
		while (running_)
		{
			...
			_HDFacesMsgPtr raw_message_ptr;
			// if the input fifo is empty, wait for producers to push items on
			{
				auto input_handle = input_fifo_.getHandle(atomics::HandleBase::NotifyType::DELAY_NOTIFY_ONE);
			...
			//            std::cout << "compressing HD faces" << std::endl;
		...
		//        std::cout << "!! HD FacesCompressTask done" << std::endl;
	}
};
...
public:
    ...
    void run()
    {
        ...
        while( running_ )
        {
			...
			else if (compressed_message_ptr->header_.payload_id_ == _FacesMsg::ID()) num_faces_++;
			else if (compressed_message_ptr->header_.payload_id_ == _HDFacesMsg::ID()) num_hd_faces_++;
...
int main( int argc, char ** argv )
{
    ...
    while( true )
    {
        try
        {
            kinect_device.initialize(true, true, true, true, true, false );
            break;
        }
        catch( KinectException & e )
        {
	...
	FacesCompressTask::_MessageCoder faces_message_coder;
	HDFacesCompressTask::_MessageCoder hd_faces_message_coder;
    SpeechCompressTask::_MessageCoder speech_message_coder;

    // set threadpool sizes
    Poco::ThreadPool read_pool( 8, 8 );
    Poco::ThreadPool compress_pool( 8 + 2 + 2 + 1 + 1 + 1 + 1 + 1, 8 + 2 + 2 + 1 + 1 + 1 + 1 + 1);
	...
	atomics::Wrapper<std::deque<_FacesMsgPtr> > faces_read_fifo;
	atomics::Wrapper<std::deque<_HDFacesMsgPtr> > hd_faces_read_fifo;
	...
	FacesReadTask faces_read_task(faces_read_fifo, kinect_device);
	HDFacesReadTask hd_faces_read_task(hd_faces_read_fifo, kinect_device);
	...
	FacesCompressTask faces_compress_task(faces_read_fifo, compress_fifo, faces_message_coder);
	HDFacesCompressTask hd_faces_compress_task(hd_faces_read_fifo, compress_fifo, hd_faces_message_coder);
	...
	for (size_t i = 0; i < 1; ++i ) compress_pool.start(faces_compress_task);
	for (size_t i = 0; i < 1; ++i)  compress_pool.start(hd_faces_compress_task);
	...
	read_pool.start(faces_read_task);
	read_pool.start(hd_faces_read_task);
    read_pool.start( speech_read_task );

    // use the main thread to produce status updates while program is running
    while( running_ )
    {
        std::cout << num_color_ << " | " << num_depth_ << " | " << num_infrared_ << " | " << num_audio_ << " | " << num_bodies_ << " | " << num_speech_ << " | " << num_faces_  << " | " << num_hd_faces_ << std::endl;
	...
	faces_read_task.running_ = false;
	hd_faces_read_task.running_ = false;
	...
	while (!faces_read_fifo->empty())
	{
		faces_read_fifo.getCondition().notify_one();
	}
	std::cout << "compress faces task done" << std::endl;

	while (!hd_faces_read_fifo->empty())
	{
		hd_faces_read_fifo.getCondition().notify_one();
	}
	std::cout << "compress hd faces task done" << std::endl;
/*
...
*/
	...
	faces_compress_task.running_ = false;
	hd_faces_compress_task.running_ = false;
	...
	faces_read_fifo.getCondition().notify_all();
	hd_faces_read_fifo.getCondition().notify_all();
